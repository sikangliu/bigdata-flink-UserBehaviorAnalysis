package com.lsk.loginfail_detect;

import com.lsk.loginfail_detect.beans.LoginEvent;
import com.lsk.loginfail_detect.beans.LoginFailWarning;
import java.net.URL;
...
public class LoginFail {
    ...
    public static class LoginFailDetectWarning0 extends KeyedProcessFunction<Long, LoginEvent, LoginFailWarning> {
        ...
        public void open(Configuration parameters) throws Exception {
            loginFailEventListState = getRuntimeContext()
                    .getListState(new ListStateDescriptor<LoginEvent>("login-fail-list", LoginEvent.class));
            ...
            if ("fail".equals(value.getLoginState())) {
                ...
                if (iterator.hasNext()) {
                    ...
                    if (value.getTimestamp() - firstFailEvent.getTimestamp() <= 2) {
                        // 1.1.1 如果在2秒之内，输出报警
                        out.collect(new LoginFailWarning(value.getUserId(), firstFailEvent.getTimestamp(),
                                value.getTimestamp(), "login fail 2 times in 2s"));
                    }
